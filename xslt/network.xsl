...
<xsl:stylesheet 
    ...
    <xsl:template match="/">
        <xsl:variable name="doc_title" select="'Netzwerk'"/>
        <html class="h-100" lang="{$default_lang}">
            <head>
                ...
                <xsl:call-template name="zoterMetaTags">
                    <xsl:with-param name="pageId" select="'network.html'"></xsl:with-param>
            ...
            <xsl:call-template name="nav_bar"/>
                ...
                <xsl:choose>
                    <xsl:when test="lower-case(normalize-space($production)) = ('1', 'true', 'yes', 'on')">
                        <script type="module" src="assets/network.js"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <script type="module" src="http://localhost:5173/@vite/client"></script>
                        <script type="module" src="http://localhost:5173/src/network.ts"></script>
                    </xsl:otherwise>
                </xsl:choose>
            </body>
        </html>
