...
from utils import listbibl_file, listorg_file, listperson_file, listplace_file
...
print("and now the orgs")

doc = TeiReader(listbibl_file)
bibl_lookup = defaultdict(list)
for x in doc.any_xpath(".//tei:bibl[@xml:id]"):
    for y in x.xpath("./tei:publisher/@key", namespaces=NSMAP):
        bibl_node = copy.deepcopy(x)
        xml_id = get_xmlid(bibl_node)
        bibl_node.attrib.pop("{http://www.w3.org/XML/1998/namespace}id", None)
        bibl_node.attrib.pop("{http://www.w3.org/XML/1998/namespace}next", None)
...
doc = TeiReader(listorg_file)

for bad in doc.any_xpath(".//tei:org[@xml:id]//tei:listBibl"):
    bad.getparent().remove(bad)

for x in doc.any_xpath(".//tei:org[@xml:id]"):
    x.attrib["n"] = make_entity_label(x.xpath("./tei:orgName", namespaces=NSMAP)[0])[0]
    xml_id = get_xmlid(x)
    bibls = bibl_lookup[xml_id]
    if bibls:
        listbibl_node = ET.SubElement(
            x, "{http://www.tei-c.org/ns/1.0}listBibl", type="published_by"
        )
        listbibl_node.attrib["n"] = f"{len(bibls)}"
        for y in bibls:
            listbibl_node.append(copy.deepcopy(y))
